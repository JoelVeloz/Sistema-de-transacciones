        ...
        <div class="relative flex items-center w-full pl-1 lg:max-w-68 sm:pr-2 sm:ml-0">
            ...
            <div class="flex items-center justify-end w-1/4 gap-3 p-1 ml-5 mr-4 gaprelative sm:mr-0 sm:right-auto">
                ...
                <div class="relative" x-data="{ open: false }">
                    {{-- <button class="align-middle rounded-full focus:shadow-outline-purple focus:outline-none"
                        @click="open = !open" @keydown.escape="open = !open" aria-label="Account" aria-haspopup="true">
                        <img class="object-cover w-8 h-8 rounded-full"
                            src="https://images.unsplash.com/photo-1502378735452-bc7d86632805?ixlib=rb-0.3.5&q=80&fm=jpg&crop=entropy&cs=tinysrgb&w=200&fit=max&s=aa3a807e1bbdfd4364d1f449eaa96d82"
                            alt="" aria-hidden="true" />
                    </button> --}}
                    <button @click="open = !open" @keydown.escape="open = !open" href="#"
                        class="relative block border border-teal-400 rounded-full">
                        <img alt="profil" src="{{ Auth::user()->profile_photo_url }}"
                            class="object-cover w-8 h-8 mx-auto rounded-full" />
                    </button>
                    <template x-if="open">
                        <ul x-show="open" x-transition:enter="ease-out duration-300"
                            x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100"
                            x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100"
                            x-transition:leave-end="opacity-0" @click.away="open = !open" {{-- @keydown.escape="open = !open" --}}
                            class="absolute right-0 w-56 p-2 mt-2 space-y-2 text-gray-600 bg-white border border-blue-400 rounded-md shadow-md dark:border-gray-700 dark:text-gray-300 dark:bg-gray-700"
                            aria-label="submenu">
                            <li class="flex">
                                <a class="inline-flex items-center w-full px-2 py-1 text-sm font-semibold transition-colors duration-150 rounded-md hover:bg-gray-100 hover:text-gray-800 dark:hover:bg-gray-800 dark:hover:text-gray-200"
                                    href="#">
